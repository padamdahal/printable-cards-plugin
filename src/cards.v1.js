...
async function loadDataInCard() {
	try {
		...
		const url =
			"../../../api/tracker/enrollments/" + enrollmentId + "?fields=enrollment,trackedEntity,createdAt,occurredAt,attributes[attribute,displayName,value],events[event,status,programStage,orgUnit,occurredAt,createdAt,completedBy,dataValues[createdAt,dataElement,value,storedBy]]";
		...
		// Get User info
		const meUrl =
			"../../../api/me.json?fields=username,firstName,surname,organisationUnits[id,name,shortName,displayName]";
						...
						if (events) {
							...
							if (section.programStage) {
								filteredEvents = events.filter(
									(ev) => ev.programStage == section.programStage,
								);
							}
							filteredEvents.forEach((event) => {
								...
								if (key === "event") {
									...
									if (dataValue) {
										...
										if (Object.hasOwn(optionSetCollection, dataValue.dataElement)) {
											...
											if (
												valueKey == "occurredAt" || valueKey == "enrolledAt"
											) {
												value = event[valueKey];
											} else {
												value = dataValue[valueKey];
								...
								if (isDate(value)) {
									value = new Date(value).toISOString().split("T")[0];
									console.log(NepaliFunctions.AD2BS(value, "YYYY-MM-DD"));
									value =
										value + "|" + NepaliFunctions.AD2BS(value, "YYYY-MM-DD");
								}
								if (value) htmlBody = htmlBody.replaceAll(placeholder, value);
							});
						}
									...
									if (
										...
										if (isDate(value)) {
											...
											value =
												value +
												"|" +
												NepaliFunctions.AD2BS(value, "YYYY-MM-DD");
										}
